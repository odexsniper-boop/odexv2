...
export async function runOos7Validation() {
    console.log('=============================================================================');
    console.log('KO V3 — V2.7 FROZEN CANDIDATE vs V2.4 BASELINE: PRE-REGISTERED OOS-7 RUN');
    console.log('=============================================================================\n');

    const oos7File = 'replay/raw_txs_oos7_final.json';
    const oos7Txs = JSON.parse(fs.readFileSync(oos7File, 'utf8'));
    const uniqueMints = Array.from(new Set(oos7Txs.map(t => t.test_mint_context)));
    const manifest = JSON.parse(fs.readFileSync('validation/v2/oos7_manifest.json', 'utf8'));

    console.log(`OOS-7 Dataset: ${oos7Txs.length} transactions across ${uniqueMints.length} tokens.`);
    console.log(`SHA-256 Seal: ${manifest.dataset_sha256}`);
    console.log(`Contamination check: PASS (0 overlaps with Dev35, OOS1-6)\n`);

    // 1. Resolve Bonding Curve PDAs and ATAs
    const curves = {};
    for (const m of uniqueMints) {
        const mintTxs = oos7Txs.filter(t => t.test_mint_context === m);
        const userCounts = {};
        const ataCounts = {};
        mintTxs.forEach(tx => {
    ...
    function simulateOrchestrator(name, OrchestratorClass, config = {}) {
        ...
        const trades = entries.map(e => {
            ...
            const isClosed = !!exit;
            const pnlSol = isClosed ? exit.realized_pnl : 0;
            const pnlPct = isClosed ? exit.final_pnl_pct * 100 : 0;
            ...
            const exitSlippageCostSol = isClosed ? (e.size * (1 + (exit.final_pnl_pct || 0))) * 0.015 : (e.size * 0.015);
            ...
            return {
                ...
                isClosed,
                status: isClosed ? 'CLOSED' : 'OPEN_AT_CUTOFF',
                pnlSol,
                pnlPct,
                executionAdjustedPnlSol,
                totalDragSol,
                exitReason: isClosed ? exit.exit_reason : 'OPEN_AT_CUTOFF',
                maePct,
                mfePct,
                thesisState: lastPos?.thesis_state,
                holdTimeSeconds: isClosed && e.timestamp ? (exit.exit_timestamp - e.timestamp) : 0
            };
        });
        ...
        const totalExecPnl = closedTrades.reduce((s, t) => s + t.executionAdjustedPnlSol, 0);
        const wins = closedTrades.filter(t => t.pnlSol > 0.00001);
        const losses = closedTrades.filter(t => t.pnlSol <= 0.00001);
        ...
        const execExpectancy = closedTrades.length > 0 ? totalExecPnl / closedTrades.length : 0;

        // Cumulative Drawdown in SOL across closed trades
        let runningPnl = 0;
        let peakPnl = 0;
        let maxDdSol = 0;
        ...
        const closedCapitalDeployed = closedTrades.reduce((s, t) => s + t.size, 0);
        const pnlPerSolAllocated = closedCapitalDeployed > 0 ? totalPnl / closedCapitalDeployed : 0;
        const lossPerSolAllocated = closedCapitalDeployed > 0 ? grossLoss / closedCapitalDeployed : 0;
        const roac = closedCapitalDeployed > 0 ? (totalPnl / closedCapitalDeployed) * 100 : 0;

        // Tier Breakdown
        const tiers = ['A+', 'A', 'B', 'C'];
        const tierAnalysis = tiers.map(tier => {
            const trs = closedTrades.filter(t => t.tier === tier);
            const cnt = trs.length;
            if (cnt === 0) return { tier, count: 0, allocationSol: 0, netPnlSol: 0, expectancy: 0, pf: 0, winRate: 0, avgMaePct: 0, avgMfePct: 0, catastrophicLosses: 0, catastrophicRatePct: 0 };
        ...
        const sortedByPnl = [...closedTrades].sort((a, b) => b.pnlSol - a.pnlSol);
        ...
        return {
            ...
            avgMaePct: closedTrades.length > 0 ? closedTrades.reduce((s, t) => s + t.maePct, 0) / closedTrades.length : 0,
            worstMaePct: closedTrades.length > 0 ? Math.min(...closedTrades.map(t => t.maePct)) : 0,
            avgMfePct: closedTrades.length > 0 ? closedTrades.reduce((s, t) => s + t.mfePct, 0) / closedTrades.length : 0,
            worstMfePct: closedTrades.length > 0 ? Math.min(...closedTrades.map(t => t.mfePct)) : 0,
            avgHoldSeconds: closedTrades.length > 0 ? closedTrades.reduce((s, t) => s + t.holdTimeSeconds, 0) / closedTrades.length : 0,
            mfeCaptureRatio,
            totalCapitalDeployed,
            closedCapitalDeployed,
            pnlPerSolAllocated,
            lossPerSolAllocated,
            roac,
            tierAnalysis,
    ...
    console.log('=============================================================================================================');
    console.log('OOS-7 VALIDATION COMPARISON: V2.4 BASELINE vs V2.7 FROZEN CANDIDATE');
    console.log('=============================================================================================================');
    console.log('Metric                                    | V2.4 Frozen Baseline       | V2.7 Frozen Candidate (Profile C)');
    console.log('-------------------------------------------------------------------------------------------------------------');
    console.log(`Total Trades (Closed / Open)              | ${(resV2_4_Run1.closedTradesCount + ' closed, ' + resV2_4_Run1.openTradesCount + ' open').padEnd(26)} | ${resV2_7_Run1.closedTradesCount} closed, ${resV2_7_Run1.openTradesCount} open`);
    console.log(`Unique Tokens Traded                      | ${resV2_4_Run1.uniqueTokensTraded.toString().padEnd(26)} | ${resV2_7_Run1.uniqueTokensTraded}`);
    console.log(`Wins / Losses (Closed)                    | ${(resV2_4_Run1.wins + ' / ' + resV2_4_Run1.losses).padEnd(26)} | ${resV2_7_Run1.wins} / ${resV2_7_Run1.losses}`);
    console.log(`Win Rate (Closed)                         | ${(resV2_4_Run1.winRate.toFixed(1) + '%').padEnd(26)} | ${resV2_7_Run1.winRate.toFixed(1)}%`);
    ...
    const comparisonJson = {
        meta: {
            validation_name: 'OOS-7 POSITION SIZING SEQUENTIAL VALIDATION',
            timestamp: new Date().toISOString(),
            dataset_file: oos7File,
            dataset_sha256: manifest.dataset_sha256,
            total_tokens: uniqueMints.length,
            total_transactions: oos7Txs.length,
    ...
    const forensicsJson = {
        meta: {
            validation_name: 'OOS-7 FORENSICS & PAIRED TRADE AUDIT',
            timestamp: new Date().toISOString(),
            dataset_file: oos7File
        },
        catastrophic_forensics: catastrophicForensics,
        winner_forensics: winnerForensics,
        paired_trades: pairedAnalysis
    };

    fs.writeFileSync('validation/v2/v2_oos7_comparison.json', JSON.stringify(comparisonJson, null, 2));
    fs.writeFileSync('validation/v2/v2_oos7_forensics.json', JSON.stringify(forensicsJson, null, 2));
    console.log('Saved validation/v2/v2_oos7_comparison.json');
    console.log('Saved validation/v2/v2_oos7_forensics.json\n');

    return { comparisonJson, forensicsJson };
}

runOos7Validation().catch(err => {
    console.error('Fatal error in OOS-7 validation:', err);
    process.exit(1);
});
