...
/**
 * Strategy Orchestrator V2.1 (Frozen Candidate)
 * 
 * Includes:
...
export class StrategyOrchestratorV2_1 {
    constructor(config = {}) {
        this.config = {
        ...
        // Patch Opportunity & Decision Engine to respect V2.1 calibrated tiers
        const originalOpp = this.decisionEngine.opportunity.evaluate.bind(this.decisionEngine.opportunity);
        this.decisionEngine.opportunity.evaluate = function (state) {
            ...
            // Revert original crude penalty
            const origPen = Math.floor(cr / 2);
            res.opportunityScore += origPen;

            // Apply V2.1 context-calibrated penalty
            let v21Pen = 0;
        ...
        // Decision logic with Tier C Probing
            ...
            if (event.classification === 'ECONOMIC_TRADE' || event.classification === 'VIRTUAL_LIQUIDITY_EVENT') {
                ...
                if (this.openPositions.has(mint)) {
                    ...
                    if (decision.DECISION === 'BUY' && ['A+', 'A', 'B', 'C'].includes(decision.TIER)) {
                        const positionId = `v2_1_${mint}_${currentTime}`;
