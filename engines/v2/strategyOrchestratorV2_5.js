...
import { MarketRegimeEngineV2_5 } from './marketRegimeEngineV2_5.js';

/**
 * Strategy Orchestrator V2.5 (Market Regime & Launch Quality Investigation)
 * 
 * Integrates:
 ...
 * - V2.4 Token Exposure Manager (Token Risk Budget & Consecutive Loss Capping)
 * - V2.5 Market Regime Engine (Causal Rolling Cross-Token Flow & Participation Filter)
 * - Hard Safety & Priority Inviolability
 * - Zero-Lookahead Compliant
 */

export class StrategyOrchestratorV2_5 {
    constructor(config = {}) {
        this.config = {
            ...
            enableRegimeFilter: config.enableRegimeFilter ?? true,
        ...
        // V2.5 Macro Market Regime Engine
        this.regimeEngine = new MarketRegimeEngineV2_5(this.config.regimeConfig);
    ...
    processTransaction(tx, mint, curvePda, curveTokenAccount) {
        ...
        for (const event of classifiedEvents) {
            if (event.classification === 'ECONOMIC_TRADE' || event.classification === 'VIRTUAL_LIQUIDITY_EVENT') {
                ...
                // Update Macro Market Regime Engine on every economic trade
                if (event.classification === 'ECONOMIC_TRADE') {
                    this.regimeEngine.processTrade(event, mint);
                }
                ...
                if (this.openPositions.has(mint)) {
                    ...
                    if (updateRes) {
                        ...
                        if (action.type === 'FULL_EXIT') {
                            ...
                            const exitEvent = {
                                ...
                                fees: 0.000005,
                                slippage: marketState.liquidityMetrics?.estimatedSlippage?.value ?? 0.015
                            };
                    ...
                    if (decision.DECISION === 'BUY' && ['A+', 'A', 'B', 'C'].includes(decision.TIER)) {
                        ...
                        // Check V2.5 Macro Market Regime Gate
                        if (this.config.enableRegimeFilter) {
                            const regimeCheck = this.regimeEngine.evaluateEntryGate(decision.OPPORTUNITY_SCORE, currentTime);
                            if (!regimeCheck.allowed) {
                                this.blockedRegimeEntries.push({
                                    token: mint,
                                    timestamp: currentTime,
                                    tier: decision.TIER,
                        ...
                        // Open New Position
                        const positionId = `v2_5_${mint}_${currentTime}`;
