    ...
    try {
      ...
      if (this.isPaperTrading) {
        ...
        const fill = {
          ...
          solSpent: solAmount,
          tokensReceived: Number(expectedTokensOut) / 1e6,
          rawTokensReceived: expectedTokensOut.toString(),
        ...
        log(`[PAPER BUY] ${fill.tokensReceived.toFixed(2)} tokens for ${solAmount} SOL (${fill.latencyMs}ms)`);
        return fill;
      }
  ...
  async sell({
    ...
    try {
      ...
      if (this.isPaperTrading) {
        const simulatedLatency = 800;
        await new Promise(resolve => setTimeout(resolve, simulatedLatency));
        const actualGrossSellProceeds = solReceived;
        const actualTransactionFees = 0.000005; // Standard 5000 lamports
        const actualNetSellProceeds = Math.max(0, actualGrossSellProceeds - actualTransactionFees);
